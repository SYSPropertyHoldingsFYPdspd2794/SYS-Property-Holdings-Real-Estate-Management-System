                    ...
                    <div class="card border-0 shadow-sm rounded-4 h-100 bg-white p-4 p-md-5">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="hc-state" class="form-select form-select-lg bg-light border-0 shadow-sm">
                                ...
                                <optgroup label="3 Federal Territories">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="range" class="form-range custom-slider" id="hc-dp-slider" min="10" max="45" step="1" value="10">
                        </div>
                ...
                <div class="col-lg-7">
                    <div class="card border-0 shadow-sm rounded-4 h-100 bg-white p-4 p-md-5">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-12 mt-4 d-flex justify-content-between align-items-center border-bottom pb-2">
                                <h6 class="fw-bold text-secondary text-uppercase tracking-wider small m-0">Dynamic Incomes & Expenses</h6>
                                <button type="button" class="btn btn-sm btn-primary rounded-pill shadow-sm fw-bold" onclick="addDynamicField()"><i class="fas fa-plus me-1"></i> Add Item</button>
                            </div>
                            <div id="dynamic-fields-container" class="col-12 m-0 p-0 row g-3 mt-2">
...
function calculateRentVsBuy() {
    ...
    // Standard Loan Math
    const dpPercent = parseFloat(document.getElementById('hc-dp-slider').value) / 100;
    const dp = propertyPrice * dpPercent;
...
function addDynamicField() {
    ...
    row.className = 'col-12 dynamic-field-row d-flex gap-2 align-items-center mb-2';
    row.innerHTML = `
        <select class="form-select bg-light w-auto dynamic-type" onchange="calculateDSR()">
        ...
        <button type="button" class="btn btn-outline-danger btn-sm" onclick="this.closest('.dynamic-field-row').remove(); calculateDSR();"><i class="fas fa-trash"></i></button>
    `;
...
function calculateDSR() {
    ...
    document.querySelectorAll('.dynamic-field-row').forEach(row => {
        const type = row.querySelector('.dynamic-type').value;
        const amount = parseFloat(row.querySelector('.dynamic-amount').value) || 0;
        if (type === 'income') dynamicIncome += amount;
        else dynamicExpense += amount;
    });

    const baseIncome = parseFloat(document.getElementById('dsr-income').value) || 0;
    const totalIncome = baseIncome + dynamicIncome;
    ...
    const lifestyleExpenses = food + ent + util + dynamicExpense;

    // Subtracted from Net Income to ensure DSR calculation is accurate
    let dsrNetIncome = totalIncome - lifestyleExpenses;
    if (dsrNetIncome <= 0) dsrNetIncome = 1; // Prevent division by zero
    ...
    const dsrLimit = dsrNetIncome < 3500 ? 0.60 : 0.70; // Bank Negara safe limits
    
    let currentDSR = 0;
    if (dsrNetIncome > 0) {
        currentDSR = (totalBankDebt / dsrNetIncome) * 100;
    }

    const maxAllowedTotalDebt = dsrNetIncome * dsrLimit;
