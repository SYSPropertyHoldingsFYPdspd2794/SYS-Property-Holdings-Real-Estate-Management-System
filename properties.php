 ...
 * DESCRIPTION: Kept 100% original top layout. Added full-width horizontal bottom cards with JPG floor plans and numeric KM proximity.
 */
...
// -------------------------------------------------------------
// DYNAMIC PROPERTIES DESCRIPTION LOGIC (FOR BOTTOM CARDS)
// -------------------------------------------------------------
...
if (!file_exists("../" . str_replace('../', '', $finalFloorPlan))) {
...
$layoutHtml = "";
if ($is_afford) {
    // IRONCLAD RULE: Gov Affordable is exactly 3 Bed 2 Bath
    $layoutHtml = '
    ...
    if ($dbType === 'commercial') {
        $layoutHtml = '
            <div class="row text-center g-4">
                ...
                <div class="col-md-4 col-6"><div class="p-4 bg-white border rounded shadow-sm"><i class="fas fa-briefcase fa-2x text-primary mb-2"></i><h5 class="fw-bold mb-0">1 Office Room</h5></div></div>
        ...
        if ($dbType === 'bungalow') {
            ...
            // Terrace Calculation based on SQFT
            $rooms = ($sqft >= 1500) ? "5" : "4";
            $baths = ($sqft >= 1500) ? "4" : "3";
            $layoutHtml = '
                <div class="row text-center g-4">
                ...
                <div class="card-body p-5">
                    ...
                    <?php if ($is_afford): ?>
                        <div class="alert alert-danger border-0 shadow-sm p-4 mb-4" style="background-color: #fff5f5; border-left: 5px solid #dc3545 !important;">
                            ...
                            <p class="mb-0 text-dark">Subsidized unit. Household income must be <strong>Below RM <?php echo number_format($property['income_limit_rm'] ?? 0); ?></strong>. Staff must verify offline.</p>
                        </div>
                    <?php endif; ?>
