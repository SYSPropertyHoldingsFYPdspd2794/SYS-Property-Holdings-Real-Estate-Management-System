...
if ($type === 'appointment') {
    ...
    $stmt = $conn->prepare("SELECT ah.*, p.project_name, p.state, p.price, p.property_code, p.income_limit_rm, c.marital_status, c.dependents_count, c.occupation, c.monthly_income 
...
// HANDLE BASIC SALARY (MONTHLY INCOME) UPDATE FOR HOUSING
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action_type'] ?? '') === 'update_income') {
    if ($type === 'housing' && $allow_document_adjustment) {
        $new_income = floatval($_POST['monthly_income'] ?? 0);
        $income_limit = floatval($data['income_limit_rm'] ?? 0);
        
        if ($new_income <= 0) {
            $error = "Please enter a valid numeric value for your Basic Salary.";
        } elseif ($new_income > $income_limit && $income_limit > 0) {
            $error = "Update Failed: The declared income (RM " . number_format($new_income, 2) . ") exceeds the property's income limit (RM " . number_format($income_limit, 2) . "). Application would be disqualified.";
        } else {
            $upd_income = $conn->prepare("UPDATE customers SET monthly_income = ? WHERE customer_id = ?");
            $upd_income->bind_param("di", $new_income, $account_id);
            if ($upd_income->execute()) {
                $success = "Basic Salary (Monthly Income) successfully updated to RM " . number_format($new_income, 2) . ".";
                $data['monthly_income'] = $new_income;
            } else {
                $error = "Database error: Unable to update income.";
            }
        }
    } else {
        $error = "Action Denied: Income adjustment is locked for this tracking status.";
    }
}
            ...
            <div class="card border-0 shadow-lg rounded-4 overflow-hidden position-relative p-4 reveal-card" style="background: #fff; border-top: 8px solid #212529 !important;">
                ...
                <div class="p-2 font-monospace fs-6">
                    ...
                    <?php if ($type === 'housing'): ?>
                        ...
                        <div class="d-flex justify-content-between mb-3">
                            <span class="text-muted">Basic Salary (Monthly):</span>
                            <span class="fw-bold text-primary">RM <?php echo number_format($data['monthly_income'], 2); ?></span>
                        </div>
                        
                        <?php if ($allow_document_adjustment): ?>
                            <form method="POST" id="updateIncomeForm" class="mt-3 p-3 bg-white border border-primary border-opacity-25 rounded-3 shadow-sm">
                                <input type="hidden" name="action_type" value="update_income">
                                <label class="form-label fw-bold small text-primary"><i class="fas fa-edit me-1"></i>Update Basic Salary</label>
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text bg-light fw-bold">RM</span>
                                    <input type="number" step="0.01" name="monthly_income" class="form-control fw-bold" value="<?php echo htmlspecialchars($data['monthly_income']); ?>" required>
                                    <button type="submit" class="btn btn-primary fw-bold px-3">Save Changes</button>
                                </div>
                            </form>
                        <?php endif; ?>
                    <?php endif; ?>
