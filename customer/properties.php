 ...
 * DESCRIPTION: Catalog with dynamic search and instant Wishlist toggle logic.
 */
...
// FETCH USER'S CURRENT WISHLIST
$wishlist_array = [];
$w_stmt = $conn->prepare("SELECT property_id FROM wishlists WHERE customer_id = ?");
$w_stmt->bind_param("i", $account_id);
$w_stmt->execute();
$w_res = $w_stmt->get_result();
while($w_row = $w_res->fetch_assoc()) {
    $wishlist_array[] = $w_row['property_id'];
}
...
if (!empty($filter_type)) { $sql .= " AND property_type = ?"; $params[] = $filter_type; $types .= "s"; }
    ...
    <div class="row">
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $is_wishlisted = in_array($row['property_id'], $wishlist_array);
                ?>
                ...
                <div class="col-lg-4 col-md-6 mb-4">
                    <div class="card h-100 border-0 shadow-sm rounded-3 overflow-hidden hover-card">
                        <div class="position-relative bg-light" style="height: 250px;">
                            ...
                            <span class="badge bg-primary position-absolute top-0 end-0 m-3 shadow z-3"><?php echo htmlspecialchars($row['property_type']); ?></span>
                            ...
                            <form method="POST" class="position-absolute top-0 start-0 m-3 z-3">
                                ...
                                <button type="submit" name="toggle_wishlist" class="btn btn-light rounded-circle shadow-sm p-0 d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;" title="Add/Remove Wishlist">
                        ...
                        <div class="card-footer bg-white border-0 py-3 text-center">
                            <small class="text-muted"><i class="fas fa-door-open me-1"></i> Available Units: <strong class="text-dark"><?php echo $availableUnits; ?></strong></small>
                        </div>
                    </div>
                </div>
                <?php
            ...
            echo '<div class="col-12 text-center py-5"><i class="fas fa-search-minus fa-3x text-muted mb-3"></i><h4 class="text-muted fw-bold">No properties found</h4><a href="properties.php" class="btn btn-outline-primary mt-2">Clear Filters</a></div>';
        }
        ?>
    </div>
</div>
